...
Status CsvScanNode::Open(RuntimeState* state) {
	...
	// Start first Row Batch
	StartNewRowBatch();

	// Get Memory for the first Row Batch
	...
	// Scan the CSV file, comma is the Delimitter
	std::string myfile = GetFilePath();
	ifstream fin(myfile.c_str(), std::ifstream::in);
	if(fin.is_open())
	{
		// Reads each line
    		...
    		while( i < materialized_slots_.size())
          	{
            	...
            	const SlotDescriptor* mydesc = materialized_slots_[i];

            	// This is required if the user queries for only specific columns in select clause
            	// In such a case we need to pass the correct matching scan value to writeSlot()
            	j= mydesc->col_pos();
            	while( j > i)
            	{
            	...
            	// Reads a column value from CSV file
            	getline(linestream,cell,',');

            	// Below is the code to handle escape comma in CSV file
            	// If the file contains 'Ahoy\, Matey!', the code will parse and extract 'Ahoy, Matey!' as a single string
            	if (cell.find("\\",cell.size()-1) != std::string::npos)
            	{
            		...
            		mycell = cell.c_str();
					mycell2 = cell2.c_str();
					fullstring = const_cast<char *>(mycell);

					//strcat requires first parameter to be char * and second parameter to be const char *
					std::strcat(fullstring,mycell2);
					cell =  (const char *) fullstring;
            	}

            	//At this stage we have the correct matching slot value
            	int len = cell.size();
            	const char * mycell = cell.c_str();
				WriteSlot(mydesc, tuple, mycell, len, pool);
            	i++;
            }

			tuple_row->SetTuple(0,tuple);

			//The newly formed row is evaluated aganist conjuncts. All where clause commands are taken care here
			if (!EvalConjuncts(conjuncts, conjuncts_.size(), tuple_row)) continue;
      		++rows_returned;

      		// Filled up current row batch or hit limit, Commit the rows to output Row Batch
      		if (UNLIKELY(rows_returned == max_tuples))
      		{
      			Status status = CommitRows(rows_returned);
      			rows_returned = 0;

      			//After commit, Get Memory for the next Row Batch
      			max_tuples = GetMemory(&pool, &tuple_, &tuple_row);
      			tuple_row_mem = reinterpret_cast<uint8_t*>(tuple_row);
    			tuple_mem = reinterpret_cast<uint8_t*>(tuple_);
  				tuple = reinterpret_cast<Tuple*>(tuple_mem);
  				continue;
      		}
      		
			 // Update the pointers for the next tuple
 	...
 	//Commit the rows of the last Row Batch
 	Status status = CommitLastRows(rows_returned);
 	fin.close();
...
Status CsvScanNode::GetNextInternal(RuntimeState* state, RowBatch* row_batch,
	...
	**/
	...
	if((materialized_row_batches_->GetSize()) > 0)
	{
